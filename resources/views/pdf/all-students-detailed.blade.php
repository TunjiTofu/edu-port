...
@foreach($students as $studentIndex => $studentData)
    ...
    {{-- Sections Breakdown --}}
    @if(isset($studentData['sections']) && count($studentData['sections']) > 0)
    ...
    @else
        <div class="section">
            <div class="section-title" style="background-color: #C00000;">NO SECTIONS DATA FOUND</div>
            <p style="padding: 10px;">Debug: Sections count = {{ isset($studentData['sections']) ? count($studentData['sections']) : 'not set' }}</p>
        </div>
    @endif
@endforeach

<div class="footer">
    <p style="margin: 0 0 3px 0;">This is an official academic report. Generated by the Academic Management System.</p>
